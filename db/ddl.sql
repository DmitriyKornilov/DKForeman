...
CREATE TABLE IF NOT EXISTS SCHEDULEMAIN (
    ...
    CYCLECOUNT   INTEGER NOT NULL DEFAULT 7
);
...
INSERT OR IGNORE INTO  SHIFTCOLORS (COLORINDEX, COLORVALUE) VALUES (-1, 14079702);
INSERT OR IGNORE INTO  SHIFTCOLORS (COLORINDEX, COLORVALUE) VALUES (0, 16777215);
INSERT OR IGNORE INTO  SHIFTCOLORS (COLORINDEX, COLORVALUE) VALUES (1, 11796479);
INSERT OR IGNORE INTO  SHIFTCOLORS (COLORINDEX, COLORVALUE) VALUES (2, 13427660);

/* КЛАССЫ СИЗ */
CREATE TABLE IF NOT EXISTS SIZCLASSES (
    CLASSID   INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    CLASSNAME TEXT    UNIQUE NOT NULL
);
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (1, 'Одежда летняя');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (2, 'Одежда зимняя');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (3, 'Одежда особая');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (4, 'Обувь летняя');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (5, 'Обувь зимняя');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (6, 'Обувь особая');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (7, 'Головные уборы');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (8, 'Средства защиты органов дыхания');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (9, 'Средства защиты органов зрения');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (10, 'Средства защиты органов слуха');
INSERT OR IGNORE INTO  SIZCLASSES (CLASSID, CLASSNAME) VALUES (11, 'Средства защиты рук');
...
CREATE TABLE IF NOT EXISTS SIZUNIT (
    ...
    UNITDIGITALCODE INTEGER UNIQUE NOT NULL,
...
/* ПЕРИОД ДЛЯ ФОРМЫ ФМУ-85 (СИЗ) */
CREATE TABLE IF NOT EXISTS SIZFMU85PERIOD (
    DOCID  INTEGER UNIQUE NOT NULL,
    PERIOD TEXT    NOT NULL,
    CONSTRAINT FK_SIZFMU85PERIOD_DOCID FOREIGN KEY (DOCID) REFERENCES SIZDOC(DOCID) ON UPDATE CASCADE ON DELETE CASCADE
);

/* НАИМЕНОВАНИЯ СИЗ */
CREATE TABLE IF NOT EXISTS SIZNAMES (
    ...
    CONSTRAINT FK_SIZNAMES_CLASSID FOREIGN KEY (CLASSID) REFERENCES SIZCLASSES(CLASSID) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZNAMES_UNITID  FOREIGN KEY (UNITID)  REFERENCES SIZUNIT(UNITID)     ON UPDATE CASCADE ON DELETE SET DEFAULT
);
...
CREATE UNIQUE INDEX IF NOT EXISTS IDX_SIZNORMITEM_ITEMNAMENORMID ON SIZNORMITEM(ITEMNAME, NORMID);
...
/* СПИСОК ОСОБЫХ СРОКОВ СЛУЖБЫ СИЗ */
CREATE TABLE IF NOT EXISTS SIZSPECLIFE (
    SPECLIFEID   INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    SPECLIFENAME TEXT    UNIQUE NOT NULL
);
INSERT OR IGNORE INTO  SIZSPECLIFE (SPECLIFEID, SPECLIFENAME) VALUES (0, '<не указан>');
INSERT OR IGNORE INTO  SIZSPECLIFE (SPECLIFEID, SPECLIFENAME) VALUES (1, 'дежурное');
INSERT OR IGNORE INTO  SIZSPECLIFE (SPECLIFEID, SPECLIFENAME) VALUES (2, 'до износа');
...
CREATE TABLE IF NOT EXISTS SIZNORMSUBITEMINFO (
    ...
    SPECLIFEID INTEGER NOT NULL DEFAULT 0,
    ORDERNUM   INTEGER NOT NULL,
    ...
    CONSTRAINT FK_SIZNORMSUBITEMINFO_NAMEID     FOREIGN KEY (NAMEID)     REFERENCES SIZNAMES(NAMEID)           ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZNORMSUBITEMINFO_SPECLIFEID FOREIGN KEY (SPECLIFEID) REFERENCES SIZSPECLIFE(SPECLIFEID)    ON UPDATE CASCADE ON DELETE SET DEFAULT
);

/* СПИСОК ПОСТУПЛЕНИЯ СИЗ НА СКЛАД */
CREATE TABLE IF NOT EXISTS SIZENTRY (
    ENTRYID     INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    NOMNUM      TEXT    NOT NULL,
    NAMEID      INTEGER NOT NULL,
    SIZEID      INTEGER NOT NULL DEFAULT 0,
    HEIGHTID    INTEGER NOT NULL DEFAULT 0,
    WEARPERCENT INTEGER NOT NULL DEFAULT 0,
    ENTRYCOUNT  INTEGER NOT NULL DEFAULT 1,
    CERTIFICATE TEXT   ,
    DOCID       INTEGER NOT NULL,
    CONSTRAINT FK_SIZENTRY_NAMEID FOREIGN KEY (NAMEID) REFERENCES SIZNAMES(NAMEID) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZENTRY_DOCID  FOREIGN KEY (DOCID)  REFERENCES SIZDOC(DOCID)    ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS STAFFTABNUM (
    ...
    CONSTRAINT FK_STAFFTABNUM_POSTID  FOREIGN KEY (POSTID)  REFERENCES STAFFPOST(POSTID)  ON UPDATE CASCADE ON DELETE SET DEFAULT
);

/* ЖУРНАЛ ВЫДАЧИ СИЗ РАБОТНИКАМ */
CREATE TABLE IF NOT EXISTS SIZSTAFFLOG (
    LOGID         INTEGER  PRIMARY KEY AUTOINCREMENT NOT NULL,
    TABNUMID      INTEGER  NOT NULL,
    GETTINGDATE   DATETIME NOT NULL,
    GETTINGDOCID  INTEGER  NOT NULL,
    GETTINGINFOID INTEGER  NOT NULL,
    NOWINFOID     INTEGER  NOT NULL,
    CONSTRAINT FK_SIZSTAFFLOG_TABNUMID      FOREIGN KEY (TABNUMID)      REFERENCES STAFFTABNUM(TABNUMID)        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZSTAFFLOG_GETTINGDOCID  FOREIGN KEY (GETTINGDOCID)  REFERENCES SIZDOC(DOCID)                ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZSTAFFLOG_GETTINGINFOID FOREIGN KEY (GETTINGINFOID) REFERENCES SIZNORMSUBITEMINFO(INFOID)   ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZSTAFFLOG_NOWINFOID     FOREIGN KEY (NOWINFOID)     REFERENCES SIZNORMSUBITEMINFO(INFOID)   ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS SIZSTAFFSIZE (
    ...
    HEADDRESS  INTEGER NOT NULL DEFAULT 0,
    MITTENS    INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS SIZSTORE (
    STOREID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    ENTRYID INTEGER NOT NULL,
    ISBUSY  INTEGER NOT NULL DEFAULT 0,
    CONSTRAINT FK_SIZSTORE_ENTRYID FOREIGN KEY (ENTRYID) REFERENCES SIZENTRY(ENTRYID) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS SIZSTOREWRITEOFF (
    ...
    CONSTRAINT FK_SIZSTOREWRITEOFF_STOREID FOREIGN KEY (STOREID) REFERENCES SIZSTORE(STOREID) ON UPDATE CASCADE ON DELETE CASCADE
);

/* ИНФОРМАЦИЯ О ВОЗВРАТЕ/ДОСРОЧНОМ СПИСАНИИ СИЗ РАБОТНИКОВ */
CREATE TABLE IF NOT EXISTS SIZSTAFFBACK (
    BACKID    INTEGER  PRIMARY KEY AUTOINCREMENT NOT NULL,
    BACKDOCID INTEGER  NOT NULL,
    BACKDATE  DATETIME NOT NULL,
    STOREID   INTEGER  NOT NULL,
    ENTRYID   INTEGER  NOT NULL DEFAULT 0,
    CONSTRAINT FK_SIZSTAFFBACK_BACKDOCID FOREIGN KEY (BACKDOCID) REFERENCES SIZDOC(DOCID)     ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZSTAFFBACK_STOREID   FOREIGN KEY (STOREID)   REFERENCES SIZSTORE(STOREID) ON UPDATE CASCADE ON DELETE CASCADE
);

/* КЛАССЫ ССО (1122н) */
CREATE TABLE IF NOT EXISTS SSOCLASSES (
    CLASSID   INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    CLASSNAME TEXT    UNIQUE NOT NULL
);
INSERT OR IGNORE INTO  SSOCLASSES (CLASSID, CLASSNAME) VALUES (1, 'Защитные средства');
INSERT OR IGNORE INTO  SSOCLASSES (CLASSID, CLASSNAME) VALUES (2, 'Очищающие средства');
INSERT OR IGNORE INTO  SSOCLASSES (CLASSID, CLASSNAME) VALUES (3, 'Восстанавливающие средства');

/* СПИСОК ДОКУМЕНТОВ ВЫДАЧИ/СПИСАНИЯ ССО */
CREATE TABLE IF NOT EXISTS SSODOC (
    DOCID   INTEGER  PRIMARY KEY AUTOINCREMENT NOT NULL,
    DOCNAME TEXT     NOT NULL,
    DOCDATE DATETIME NOT NULL,
    DOCNUM  TEXT     NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS IDX_SSODOC_DOCNAMENUMDATE ON SSODOC(DOCNAME, DOCDATE, DOCNUM);

/* ЕДИНИЦЫ ИЗМЕРЕНИЯ ССО */
CREATE TABLE IF NOT EXISTS SSOUNIT (
    UNITID          INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    UNITNAME        TEXT    UNIQUE NOT NULL,
    UNITDIGITALCODE INTEGER NOT NULL,
    UNITSTRINGCODE  TEXT    UNIQUE NOT NULL
);
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (0, '<не указана>', 0, ' ');
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (1, 'Миллилитр', 111, 'мл');
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (2, 'Грамм', 163, 'г');
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (3, 'Кубический сантиметр', 111, 'куб. см');
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (4, 'Литр', 112, 'л');
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (5, 'Кубический дециметр', 112, 'куб. дм');
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (6, 'Килограмм', 166, 'кг');
INSERT OR IGNORE INTO  SSOUNIT (UNITID, UNITNAME, UNITDIGITALCODE, UNITSTRINGCODE) VALUES (7, 'Миллиграмм', 161, 'мг');

/* ПЕРИОД ДЛЯ ФОРМЫ ФМУ-85 (ССО) */
CREATE TABLE IF NOT EXISTS SSOFMU85PERIOD (
    DOCID  INTEGER UNIQUE NOT NULL,
    PERIOD TEXT    NOT NULL,
    CONSTRAINT FK_SSOFMU85PERIOD_DOCID FOREIGN KEY (DOCID) REFERENCES SSODOC(DOCID) ON UPDATE CASCADE ON DELETE CASCADE
);

/* НАИМЕНОВАНИЯ ССО */
CREATE TABLE IF NOT EXISTS SSONAMES (
    NAMEID  INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    SSONAME TEXT    UNIQUE NOT NULL,
    CLASSID INTEGER NOT NULL,
    UNITID  INTEGER NOT NULL DEFAULT 0,
    METHOD  INTEGER NOT NULL DEFAULT 0,
    CONSTRAINT FK_SSONAMES_CLASSID FOREIGN KEY (CLASSID) REFERENCES SSOCLASSES(CLASSID) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SSONAMES_UNITID  FOREIGN KEY (UNITID)  REFERENCES SSOUNIT(UNITID)     ON UPDATE CASCADE ON DELETE SET DEFAULT
);

/* СПИСОК ТИПОВЫХ НОРМ ВЫДАЧИ ССО */
CREATE TABLE IF NOT EXISTS SSONORM (
    NORMID      INTEGER  PRIMARY KEY AUTOINCREMENT NOT NULL,
    NORMNAME    TEXT     UNIQUE NOT NULL,
    TYPICALNAME TEXT    ,
    BEGINDATE   DATETIME NOT NULL,
...
/* ПУНКТЫ ТИПОВОЙ НОРМЫ ВЫДАЧИ ССО */
CREATE TABLE IF NOT EXISTS SSONORMITEM (
    ITEMID   INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    ITEMNAME TEXT    NOT NULL,
    NORMID   INTEGER NOT NULL,
    NAMEID   INTEGER NOT NULL,
    VOLUME   INTEGER NOT NULL DEFAULT 100,
    CONSTRAINT FK_SSONORMITEM_NORMID FOREIGN KEY (NORMID) REFERENCES SSONORM(NORMID)  ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SSONORMITEM_NAMEID FOREIGN KEY (NAMEID) REFERENCES SSONAMES(NAMEID) ON UPDATE CASCADE ON DELETE CASCADE
);
CREATE UNIQUE INDEX IF NOT EXISTS IDX_SSONORMITEM_ITEMNAMENORMID ON SSONORMITEM(ITEMNAME, NORMID);

/* СООТВЕТСТВИЕ ПУНКТОВ ТИПОВОЙ НОРМЫ ВЫДАЧИ ССО ДОЛЖНОСТИ РАБОТНИКА */
CREATE TABLE IF NOT EXISTS SSONORMITEMPOST (
    ITEMID INTEGER NOT NULL,
    POSTID INTEGER NOT NULL,
    CONSTRAINT FK_SSONORMITEMPOST_ITEMID FOREIGN KEY (ITEMID) REFERENCES SSONORMITEM(ITEMID) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SSONORMITEMPOST_POSTID FOREIGN KEY (POSTID) REFERENCES STAFFPOST(POSTID)   ON UPDATE CASCADE ON DELETE CASCADE
);

/* СПИСОК ПОСТУПЛЕНИЯ ССО НА СКЛАД */
CREATE TABLE IF NOT EXISTS SSOENTRY (
    ENTRYID     INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    NOMNUM      TEXT    NOT NULL,
    NAMEID      INTEGER NOT NULL,
    CERTIFICATE TEXT   ,
    ENTRYVOLUME INTEGER NOT NULL DEFAULT 100,
    ENTRYCOUNT  INTEGER NOT NULL DEFAULT 1,
    DOCID       INTEGER NOT NULL,
    CONSTRAINT FK_SSOENTRY_NAMEID FOREIGN KEY (NAMEID) REFERENCES SSONAMES(NAMEID) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SSOENTRY_DOCID  FOREIGN KEY (DOCID)  REFERENCES SSODOC(DOCID)    ON UPDATE CASCADE ON DELETE CASCADE
);

/* СПИСОК ССО НА СКЛАДЕ */
CREATE TABLE IF NOT EXISTS SSOSTORE (
    STOREID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    ENTRYID INTEGER NOT NULL,
    ISBUSY  INTEGER NOT NULL DEFAULT 0,
    CONSTRAINT FK_SSOSTORE_ENTRYID FOREIGN KEY (ENTRYID) REFERENCES SSOENTRY(ENTRYID) ON UPDATE CASCADE ON DELETE CASCADE
);

/* ЖУРНАЛ ВЫДАЧИ СИЗ РАБОТНИКАМ */
CREATE TABLE IF NOT EXISTS SSOSTAFFLOG (
    LOGID        INTEGER  PRIMARY KEY AUTOINCREMENT NOT NULL,
    TABNUMID     INTEGER  NOT NULL,
    GETTINGDATE  DATETIME NOT NULL,
    GETTINGDOCID INTEGER  NOT NULL,
    ITEMID       INTEGER  NOT NULL,
    STOREID      INTEGER  NOT NULL,
    CONSTRAINT FK_SSOSTAFFLOG_TABNUMID     FOREIGN KEY (TABNUMID)     REFERENCES STAFFTABNUM(TABNUMID) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SSOSTAFFLOG_GETTINGDOCID FOREIGN KEY (GETTINGDOCID) REFERENCES SSODOC(DOCID)         ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SSOSTAFFLOG_ITEMID       FOREIGN KEY (ITEMID)       REFERENCES SSONORMITEM(ITEMID)   ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SSOSTAFFLOG_STOREID      FOREIGN KEY (STOREID)      REFERENCES SSOSTORE(STOREID)     ON UPDATE CASCADE ON DELETE CASCADE
);

/* ИНФОРМАЦИЯ ПО ЗАПИСИ ЖУРНАЛА ВЫДАЧИ СИЗ (СПИСОК ВЫДАННОГО) */
CREATE TABLE IF NOT EXISTS SIZSTAFFLOGINFO (
    LOGINFOID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    LOGID     INTEGER NOT NULL,
    STOREID   INTEGER NOT NULL,
    CONSTRAINT FK_SIZSTAFFLOGINFO_LOGID   FOREIGN KEY (LOGID)   REFERENCES SIZSTAFFLOG(LOGID)   ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT FK_SIZSTAFFLOGINFO_STOREID FOREIGN KEY (STOREID) REFERENCES SIZSTORE(STOREID)    ON UPDATE CASCADE ON DELETE CASCADE
);
